...
$mainFolder    = 'svcpadmin/albums';   // folder where your albums are located - relative to root
...
if (!isset($_GET['album'])) {
    ...
    foreach($folders as $album) {
	    ...
	    if(!in_array($album, $ignore)) {    
		   ...
		   //$rand_dirs = glob($mainFolder.'/'.$album.'/thumbs/*.*', GLOB_NOSORT);
                   $rand_dirs = glob($mainFolder.'/'.$album.'/thumbs/*.*',0);
                   
                   $rand_pic  = $rand_dirs[array_rand($rand_dirs)];
                   //$rand_pic  = $rand_dirs[$rand_dirs];
     ...
     if( count($albums) == 0 ) {
        ...
        if(isset($_GET['p'])) {
	     ...
	     echo '<div class="titlebar">
                 <div class="float-left"><span class="title">Photo Gallery</span> - '.count($albums). ' Albums</div>
   ...
   echo '<div class="titlebar">
        <div class="float-left"><span class="title">'. $_GET['album'] .'</span> - '.count($files).' images</div>
         <div class="float-right"><a href="'.$_SERVER['PHP_SELF'].'">Back</a></div>  
        </div>';	  
   echo '<div class="clear"></div>';
